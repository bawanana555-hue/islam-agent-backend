// islam_data.js — VERSION CORRIGÉE
// Chargé en script classique (<script src="islam_data.js">), donc on expose
// les données et la recherche sur window pour qu'index.html puisse les utiliser.
...
window.islamData = {
  ...
  matriceEpistemique: {
    sourcesPrimaire: [
      { nom: "Le Coran (Al-Qur'an)", nature: "Révélation directe (Wahy Matluw)", statut: "Source suprême et immuable" },
      { nom: "La Sunnah", nature: "Propos, actes et approbations du Prophète (Wahy Ghayr Matluw)", statut: "Explication et application du Coran" }
    ],
    sourcesSecondaires: [
...
// Recherche textuelle dans la base locale — exposée globalement
window.chercherDansBase = function (requete) {
  const terme = String(requete || '').toLowerCase();
  const resultats = [];
  const data = window.islamData;
  if (!data) return resultats;

  data.matriceEpistemique.sourcesSecondaires.forEach(function (item) {
    if (item.nom.toLowerCase().includes(terme) || item.description.toLowerCase().includes(terme)) {
      resultats.push({ type: 'source', titre: item.nom, contenu: item.description });
    }
  });
  data.matriceEpistemique.outilsCritiques.forEach(function (item) {
    if (item.concept.toLowerCase().includes(terme) || item.role.toLowerCase().includes(terme)) {
      resultats.push({ type: 'outil', titre: item.concept, contenu: item.role });
    }
  });
  return resultats;
};
